//			Sumovore_lineWanderer.c
//
// Sumovore Line Follower with Line Sensor Calibration, Obstacle Avoidance
// and Lost Wandering
//
// For the Sumovore mobile robot with the Atmel ATmega8(L) Brainboard add-on
// Compiled using GNU GCC with avr-libc 1.2.3 (from WinAVR-20050214)
//
// written by Justin Lee, Smart Devices Lab, QUT Nov. 2005
//
// Demonstrates behaviour-based robotics on the Solarbotics Sumovore robot,
// along with the use of various features of the Atmel AVR Microcontroller,
// such as pulse-width modulation (PWM), analog to digital conversion (ADC),
// timer interrupts, and use of IO ports.
//
// Note: code has been written assuming a 1 MHz MCU CLK, if this is not the
// case, then CLK prescalers will need to be modified to suit actual frequency
//
//
//			Robot Behaviour Outline
//
// On startup, there is a 5 second countdown (with red LED indicators),
// for robot placement, prior to line sensor calibration.
//
// Then the green LED is lit indicating robot should be placed on a white
// background, and the center red LED is lit indicating that about to read
// center line sensor. The background surface signal magnitude (0-8) is
// indicated with the red LEDs (none = 0, L -> R = 1-5, R -> L = 6-8), then
// green LED is flashed to indicate move robot center sensor onto black line,
// center red LED indicates reading center line sensor, and then the black
// line signal magnitude (0-8) is indicated with red LEDs. Based on this,
// the black line threshold is calculated as the rounded up midpoint between
// the two levels (or if level of black=white is the same, such as didn't
// present the background and line to the robot, then a default level of
// 128 = magnitude 8, is chosen by default) and is displayed with the red LEDs
// along with a flashing green LED to indicate this is the threshold.
//
// After this, there is another 5 second countdown (with red LED indicators),
// for placing the robot on the line so that it can start line following.
// The line following starts, with obstacle avoidance, triggered by IR
// detectors over-riding other behaviours. While presented with a line, the
// robot will attempt to follow it, however, if the robot doesn't see a line
// for a given amount of time, it is considered lost, and then begins a
// random wandering phase until a line is found.


/*

   Sumovore pinout for the ATmega8(L) Microcontroller in a PDIP package

               +---------------+
         reset |  1     PC5 28 | N.C.
      IR left  |  2 PD0 PC4 27 | Edge sensor right        (ADC4)
      IR right |  3 PD1 PC3 26 | Edge sensor center right (ADC3)
     (L) LED 1 |  4 PD2 PC2 25 | Edge sensor center       (ADC2)
         LED 2 |  5 PD3 PC1 24 | Edge sensor center left  (ADC1)
     (C) LED 3 |  6 PD4 PC0 23 | Edge sensor left         (ADC0)
           VCC |  7         22 | GND
           GND |  8         21 | AREF
        xtal 1 |  9 PB6     20 | AVCC
        xtal 2 | 10 PB7 PB5 19 | right motor direction
         LED 4 | 11 PD5 PB4 18 | left motor direction
     (R) LED 5 | 12 PD6 PB3 17 | Servo 1 / green LED
       Servo 2 | 13 PD7 PB2 16 | right motor enable/PWM   (OC1B)
       Servo 3 | 14 PB0 PB1 15 | left motor enable/PWM    (OC1A)
               +---------------+

 */

// for 8- and 16-bit unsigned int types (uint8_t uint16_t)
#include <stdint.h>

// (IO port) register definitions, IO and interrupt signal bit definitions
#include <avr/io.h>

// Interrupt registers and bit definitions, sei() and cli() macros
#include <avr/interrupt.h>

// Macros for writing interrupt handler functions
#include <avr/signal.h>


// the following useful macro defns are included via avr/io.h
// (which includes them from avr/sfr_defs.h)
//
// _BV(bit)
//	Converts 'bit' number (0-7) into a bit vector (a byte), with a
//	single bit set for setting the specified bit in a port or register.
//	Equivalent to: (1 << bit)
//	OR with port/register to set bit. ie: REG |= _BV(bit);
//	AND inverse with port/register to clear bit. ie: REG &= ~(_BV(bit));
//
// bit_is_set(sfr, bit)
// 	Test if bit 'bit' in IO register 'sfr' is set. returns 1 on set
// 	Equivalent to: REG & (1 << bit)
//
// bit_is_clear(sfr, bit)
//	Test if bit 'bit' in IO register 'sfr' is clear. returns 1 on clear
// 	Equivalent to: ~(REG & (1 << bit))
//
// loop_until_bit_is_set(sfr, bit)
//	Wait until bit 'bit' in IO register sfr is set
// 	Equivalent to: while (~(REG & (1 << bit))) {}
//
// loop_until_bit_is_clear(sfr, bit)
//	Wait until bit 'bit' in IO register sfr is clear
// 	Equivalent to: while ((REG & (1 << bit))) {}
//


// define CLK frequency as 1 MHz
#define MCU_FREQ		1000000UL

// define mnemonics for IO ports
// note that IR detectors are inputs so use PIND (in) not PORTD (out)
#define MOTORPORT	PORTB
#define GREENPORT	PORTB
#define REDPORT		PORTD
#define IRPIN		PIND

// define mnemonics for IO pins
// note Left to Right defined looking from the back of the sumovore
#define GREENLED	PB3
#define REDLED_L	PD2
#define REDLED_ML	PD3
#define REDLED_C	PD4
#define REDLED_MR	PD5
#define REDLED_R	PD6
#define LMOTOR_EN	PB1
#define RMOTOR_EN	PB2
#define LMOTOR_FORWARD	PB4
#define RMOTOR_FORWARD	PB5

// define mnemonics for the ADC channels corresponding to the edge sensors
// (L to R from back of sumovore)
#define LINESENSOR_L	0
#define LINESENSOR_ML	1
#define LINESENSOR_C	2
#define LINESENSOR_MR	3
#define LINESENSOR_R	4

// define mnemonics for IR detectors at front of sumovore
#define IR_L	PD0
#define IR_R	PD1

// define mnemonics for PWM duty cycle registers
#define LMOTOR_DUTY	OCR1A
#define RMOTOR_DUTY	OCR1B

// define some motor speed constants
#define FAST		0xFF
#define MEDIUMFAST	0xE0
#define MEDIUM		0xD0
#define MEDIUMSLOW	0xC0
#define SLOW		0xB0
#define VSLOW		0xA0
#define STOPPED		0x00

// default black line sensitivity
#define BLACKDEFAULT	128

// timeout for time elapsed since seeing white line before considered lost
// defined as seconds * 100, for 10ms resolution timer. 300 = 3 sec
#define LOST_TIMEOUT	300


// Timer/Counter2 Compare Match interrupt handler definition
// Warning: use signal names from avr/iom8.h NOT the datasheet (p. 44)!
INTERRUPT(SIG_OUTPUT_COMPARE2);


// function definitions
uint8_t calibrateLineSensitivity(void);
uint8_t cointoss(void);
void countdown(void);
void displayvalue(uint8_t value, uint8_t pause);
void flashgreen(uint8_t secs);
void flashred(uint8_t ledno, uint8_t secs);
uint8_t getlevel(uint8_t value);
void initialise(void);
void linefollower(uint8_t lineThreshold);
void lineResponseMeter(void);
void outlevel(uint8_t level);
uint8_t readADC(uint8_t channel);
void redsoff(void);
void setup10msTimer(void);
void setupADC(void);
void setupIO(void);
void setupPWM(void);
void testIRdetectors(void);
void waitsecs(uint8_t secs);
void waitMS(uint16_t msec);
void waitUS(uint16_t usec);


// define the 10ms resolution tick counter that is updated by the
// timer interrupt handler. the "volatile" keyword tells the compiler
// that tickcnt can change unexpectedly (i.e. during an interrupt).
volatile uint16_t tickcnt;



/**************************** Main Program **********************************/



int main(void)
{
  uint8_t line_threshold;

  initialise();				// initialises IO, ADC, PWM, timer

  countdown();				// 5 sec countdown to place bot

  // uncomment one of the following two if want to use bot as a
  // meter for measuring line sensor responses to various surfaces
  // lineResponseMeter();		// 8-bit meter for all line sensors
  // levelmeter(LINESENSOR_C);		// magnitude meter for a line sensor 

  // uncomment to just test the IR detectors
  //testIRdetectors();			// indicate what IRs are detecting

  // perform calibration for sensitivity at detecting lines
  line_threshold=calibrateLineSensitivity();

  countdown();				// 5 sec countdown to place bot
  linefollower(line_threshold);		// set the bot loose!

  // never reach here, it's just to keep compiler happy
  return 0;
}



/*
 *  Basic Sumovore Line Following with Obstacle Avoidance Behaviour function.
 *
 *  Line following behaviour is based on Solarbotics Sumoline.c by Bob Cook
 *  (based on original by Grant McKee), http://www.solarbotics.com, 2005.
 *
 *  Obstacle Avoidance behaviour takes precedence over Line Following and
 *  Random Wandering.
 *
 *  Obstacle Avoidance behaviour (green LED on):
 *  	- if left and right IR sensors detect an obstacle, reverse and
 *  	  change course
 *  	- if left IR sensor detects an obstacle, back away to face right
 *  	- if right IR sensor detects an obstacle, back away to face left
 *  	- otherwise, perform line following
 *
 *  Line Follower behaviour (green LED off):
 *      - If center sensor sees black, go straight forward
 *      - If center right sensor sees black, turn towards the right
 *      - If center left sensor sees black, turn towards the left
 *      - If far right sensor sees black, turn sharp right
 *      - If far left sensor sees black, turn sharp left
 *      - If all is white and timeout occurs, perform random wandering
 *
 *   Random Wandering behaviour (green LED flashing):
 *   	- repeating 3 second cycle, that can be broken off at any time, to
 *   	  resume line following, by detection of a white line, and is
 *   	  over-ridden by obstacle avoidance
 *   		- go straight ahead for 2 seconds
 *   		- after 2 seconds turn left or right randomly for 1 second
 *
 */

void linefollower(uint8_t lineThreshold)
{
  // this is used for timing in the random wandering behaviour when lost
  uint8_t lostsecs=0;

  // start both motors forward at medium speed
  MOTORPORT |= _BV(LMOTOR_FORWARD) | _BV(RMOTOR_FORWARD);
  LMOTOR_DUTY = MEDIUMFAST;
  RMOTOR_DUTY = MEDIUMFAST;

  tickcnt=0;				// start 'lost' timer

  // loop forever, following the black line while avoiding obstacles
  // note that obstacle avoidance takes precendence, and if any obstacles
  // are detected, line following is skipped
  for (;;) {

    if (bit_is_clear(IRPIN,IR_L) && bit_is_clear(IRPIN,IR_R)) {
    // if detect obstacle in front
    // reverse motors, and go full speed backwards for 5ms
    // then sharp turn left or right randomly
    // and finally resume forward motion at medium speed
      GREENPORT |= _BV(GREENLED);		// turn green LED on
      MOTORPORT &= ~(_BV(LMOTOR_FORWARD) | _BV(RMOTOR_FORWARD));
      RMOTOR_DUTY = LMOTOR_DUTY = FAST;
      waitMS(500);
      if (cointoss()) {			// turn left or right randomly
        RMOTOR_DUTY = STOPPED;
      } else {
        LMOTOR_DUTY = STOPPED;
      }
      waitMS(500);
      RMOTOR_DUTY = LMOTOR_DUTY = MEDIUM;
      MOTORPORT |= _BV(LMOTOR_FORWARD) | _BV(RMOTOR_FORWARD);
      continue;
    } else if (bit_is_clear(IRPIN,IR_L)) {
    // if detect obstacle to left
    // stop left motor, and reverse right motor full speed, to turn away
    // then resume forward motion at medium speed
      GREENPORT |= _BV(GREENLED);		// turn green LED on
      LMOTOR_DUTY = STOPPED;
      RMOTOR_DUTY = FAST;
      MOTORPORT &= ~(_BV(RMOTOR_FORWARD));
      waitMS(500);
      RMOTOR_DUTY = LMOTOR_DUTY = MEDIUMFAST;
      MOTORPORT |= _BV(RMOTOR_FORWARD);
      continue;
    } else if (bit_is_clear(IRPIN,IR_R)) {
    // if detect obstacle to right
    // stop right motor, and reverse left motor full speed, to turn away
    // then resume forward motion at medium speed
      GREENPORT |= _BV(GREENLED);		// turn green LED on
      RMOTOR_DUTY = STOPPED;
      LMOTOR_DUTY = FAST;
      MOTORPORT &= ~(_BV(LMOTOR_FORWARD));
      waitMS(500);
      RMOTOR_DUTY = LMOTOR_DUTY = MEDIUMFAST;
      MOTORPORT |= _BV(LMOTOR_FORWARD);
      continue;
    } else {
      if (tickcnt<LOST_TIMEOUT) {
        GREENPORT &= ~(_BV(GREENLED));		// turn green LED off
      }
    }


    // if detect line with outer right line sensor then right red LED off
    // do a sharp turn to the right by reversing right motor for 1 MS
    // otherwise turn right red LED on
    if (readADC(LINESENSOR_R) >= lineThreshold) {
      tickcnt=0;				// restart lost timer
      REDPORT &= ~(_BV(REDLED_R));
      MOTORPORT &= ~(_BV(RMOTOR_FORWARD));
      waitMS(1);
    } else {
      REDPORT |= _BV(REDLED_R);
    }

    // if detect black with mid right line sensor then mid-right red LED off
    // and do a turn towards the right by slowing right motor for 1 MS
    // otherwise turn mid-right red LED on
    if (readADC(LINESENSOR_MR) >= lineThreshold) {
      tickcnt=0;				// restart lost timer
      REDPORT &= ~(_BV(REDLED_MR));
      MOTORPORT |= _BV(LMOTOR_FORWARD) | _BV(RMOTOR_FORWARD);
      RMOTOR_DUTY = MEDIUM;
      waitMS(1);
    } else {
      REDPORT |= _BV(REDLED_MR);
    }

    // if detect black with center line sensor then center red LED off
    // and go straight ahead full speed
    // otherwise turn center red LED on
    if (readADC(LINESENSOR_C) >= lineThreshold) {
      tickcnt=0;				// restart lost timer
      REDPORT &= ~(_BV(REDLED_C));
      MOTORPORT |= _BV(LMOTOR_FORWARD) | _BV(RMOTOR_FORWARD);
      RMOTOR_DUTY = FAST;
      LMOTOR_DUTY = FAST;
    } else {
      REDPORT |= _BV(REDLED_C);
    }

    // if detect black with mid left line sensor then mid-left red LED off
    // and do a turn towards the left by slowing left motor for 1 MS
    // otherwise turn mid-left red LED on
    if (readADC(LINESENSOR_ML) >= lineThreshold) {
      tickcnt=0;				// restart lost timer
      REDPORT &= ~(_BV(REDLED_ML));
      MOTORPORT |= _BV(LMOTOR_FORWARD) | _BV(RMOTOR_FORWARD);
      LMOTOR_DUTY = MEDIUM;
      waitMS(1);
    } else {
      REDPORT |= _BV(REDLED_ML);
    }

    // if detect line with outer left line sensor then left red LED off
    // do a sharp turn to the left by reversing left motor for 1 MS
    // otherwise turn left red LED on
    if (readADC(LINESENSOR_L) >= lineThreshold) {
      tickcnt=0;				// restart lost timer
      REDPORT &= ~(_BV(REDLED_L));
      MOTORPORT &= ~(_BV(LMOTOR_FORWARD));
      waitMS(1);
    } else {
      REDPORT |= _BV(REDLED_L);
    }


    // if haven't seen a line in a long time, wander randomly
    if (tickcnt>=LOST_TIMEOUT) {
      // ensure that motors going in a forward direction
      MOTORPORT |= _BV(LMOTOR_FORWARD) | _BV(RMOTOR_FORWARD);

      // NOTE: don't use this tickcnt % interval approach for
      // time-critical behaviour, as there is no guarantee that
      // this code will be executed within every 10ms timer2 tick!

      if (tickcnt%20==0) {			// every ~200ms
        GREENPORT ^= _BV(GREENLED);		// flash green LED
      }

      // here implement a 3 second cycle of  2 secs straight ahead,
      // 1 sec turning left or right randomly, .. repeat while lost
      if (tickcnt>=LOST_TIMEOUT+200 && lostsecs<3) { // after 2 secs
	lostsecs=3;
	if (cointoss()) {			// randomly turn right
	  RMOTOR_DUTY = SLOW;
	  LMOTOR_DUTY = MEDIUMFAST;
	} else {				// or turn left
	  LMOTOR_DUTY = SLOW;
	  RMOTOR_DUTY = MEDIUMFAST;
	}
      }
      // then after a second of turning resume a straight line for next 2 secs
      if (tickcnt>=LOST_TIMEOUT+300 && lostsecs<4) { // after 3 secs
	LMOTOR_DUTY = RMOTOR_DUTY = MEDIUMFAST; 
	lostsecs=5;
      }
      // ready to repeat 3 second cycle
      if (tickcnt>=LOST_TIMEOUT+300) {
        tickcnt-=300;
	lostsecs=0;
      }

    } else { // if not lost
      GREENPORT &= ~(_BV(GREENLED));		// ensure green LED is off
    }

    // and repeat again
  }

  // this point is never reached!
}



/**************************** Utility Code **********************************/



// countdown from 1-5 seconds
void countdown()
{
  // ensure all red leds are off
  redsoff();

  REDPORT |= _BV(REDLED_L);
  waitsecs(1);
  REDPORT |= _BV(REDLED_ML);
  waitsecs(1);
  REDPORT |= _BV(REDLED_C);
  waitsecs(1);
  REDPORT |= _BV(REDLED_MR);
  waitsecs(1);
  REDPORT |= _BV(REDLED_R);
  waitsecs(1);

  // and now turn them all off again
  redsoff();
}



// flash green LED for so many seconds
void flashgreen(uint8_t secs) 
{
  // convert secs to 250MS, as flash 4 times a second
  secs <<= 2;
  while(secs--) {
    GREENPORT ^= _BV(GREENLED);
    waitMS(250);
  }
}



// flash red LED for so many seconds, ledno is from 2-6 (left to right)
void flashred(uint8_t ledno, uint8_t secs) 
{
  // convert secs to 250MS, as flash 4 times a second
  secs <<= 2;

  while(secs--) {
    REDPORT ^= _BV(ledno);	// cheat! as avr/iom8.h defines PDx as x
    waitMS(250);
  }
}



// turn off all the red leds
void redsoff()
{
  REDPORT &= ~( _BV(REDLED_L) | _BV(REDLED_ML)
               | _BV(REDLED_C) | _BV(REDLED_MR)
               | _BV(REDLED_R) );
}



// wait in a loop for the specified number of seconds
void waitsecs(uint8_t secs)
{
  while(secs--) { waitMS(1000); }
}



// uses wait in a loop for the specified millisecs
void waitMS(uint16_t msec)
{
  while(msec--) { waitUS(1000); }
}



// this provides an effectively random value of 0 or 1,
// based on the PWM timer counter 1 register (TCNT1),
// which is updated every 16 Usec.
uint8_t cointoss()
{
  return TCNT1%2;
}



/************************* Interrupt Handler Code **************************/



// Timer/Counter2 Compare Match interrupt handler
// note that tickcnt is uint16_t, so with an interrupt every 10ms,
// it can count for around 10:55sec before resetting
// Note use of INTERRUPT() vs SIGNAL(), this is as don't care if get preempted
// as this is not critical (and 10ms is plenty of time to handle it)
INTERRUPT(SIG_OUTPUT_COMPARE2)
{
  ++tickcnt;				// note ignoring overflow
}



/************************** ADC Interface Code **************************/



// read from ADC channel, and return most significant 8 bits of conversion:
//  - uses same ADMUX settings as in setupADC() (see above), but also
//    select ADC channel ADC0-ADC5 (which use pins PC0-PC5)
//  - start ADC conversion by setting ADSC flag in ADCSRA, noting that
//	- ADC detects ADCSRA settings on next ADC clk rising edge
//	- sample and hold then starts after 1.5 ADC clks
//	- complete conversion takes 13 ADC clks (including sample and hold)
//  - then wait for conversion to complete, indicated by ADSC flag being reset
//  - read most significant 8 bits of conversion result in ADCH
uint8_t readADC(uint8_t channel)
{
  // as not re-setting ADMUX settings, need to clear out the previous channel
  // setting (low 4 bits) first, and then set the new ADC channel
  ADMUX  &= ~0xF;
  ADMUX  |= channel;

  ADCSRA |= (1 << ADSC);		// start conversion
  waitUS(16);				// wait 1 ADC clk cycle, and then
  while (ADCSRA & (1 << ADSC));		// wait for conversion to complete

  // return most significant 8 bits of conversion
  return ADCH;
}



/****************************** Setup Code **********************************/



// initialises IO ports, ADC conversion, PWM and Timer 2 interrupts
void initialise()
{
  cli();				// turn interrupts off for setup
  setupIO();				// setup IO ports
  setupADC();				// and ADC for line sensors
  setupPWM();				// prepare PWM for motor speed control
  setup10msTimer();			// start up timer & enable interrupts
  // sei();				// re-enable interrupts
}



// to use IO pins on the AVR need to tell it which are inputs and which
// are outputs. this is done by setting the appropriate bits in the
// data direction registers (DDRx) to 0's for inputs and 1's for outputs.
// the LEDs and motors are outputs, so their DDRx bits are specifically set,
// while the ADC channel input pins are implicitly set to 0's
void setupIO()
{
  // red LEDs are on port D
  DDRD = (1 << REDLED_L) | (1 << REDLED_ML)
       | (1 << REDLED_C) | (1 << REDLED_MR)
       | (1 << REDLED_R);

  // green LED, left and right motor enable and direction are on port B
  DDRB = (1 << GREENLED)
       | (1 << LMOTOR_EN ) | (1 << RMOTOR_EN)
       | (1 << LMOTOR_FORWARD) | (1 << RMOTOR_FORWARD);
}



// set up PWM for use with channels A and B (PB1 and PB2), corresponding to
// the left and right motor enable pins, for controlling each motor's speed,
// with the speed of the motor being determined by the duty cycle, specified
// in the OCR1A (left motor) and OCR1B (right motor) registers.
// note: this function requires setupIO() called beforehand to set motor
// pins PB1 and PB2 as outputs
//  - Timer1 (16-bit timer) needs to be set up, with clock source (here
//    CPU clk without prescaler)
//  - PWM channels A and B need to be set up:
//      - Fast PWM mode selected (single slope, that is high - low, and
//        repeat on next cycle, giving hi - low - hi ..)
//        with 8 bit timer resolution
//  	- Inverting compare output mode is selected here, meaning that
//  	  higher values of OCR1x result in increased duty cycles
//  	  (waveform starts high and then goes low)
//  - set initial values for duty cycles (here 0 for motor off)
void setupPWM()
{
  // setup clk src as CPU clk
  TCCR1B = (1 << CS10);

  // select Fast PWM with 8 bit timer resolution (0-255)
  TCCR1B |= (1 << WGM12);
  TCCR1A = (1 << WGM10);

  // select Inverted Compare mode (start with high signal on each cycle),
  // so that OCR1x will specify the length of duty cycle (rather than inverse)
  TCCR1A |= (1 << COM1A1) | (1 << COM1B1);

  // set default values of zero (motors off) to duty cycles
  OCR1A=OCR1B=0;
}



// sets up ADC for use:
// - setup ADC Multiplexor Select register (ADMUX):
//   - select Vref (with REFSx). For the Sumovore using AVCC
//     with external capacitor at AREF pin, select this with REFS0
//   - (optional) select left adjust (ADLAR) if only want to use
//     the most significant 8 bits of ADC conversions. This is
//     suitable for the edge detectors, as only testing for the
//     magnitude of their signal, higher precision won't help.
// - setup ADC Ctrl & Status register (ADCSRA):
//   - the clk prescaler needs to be set (with ADPSx), to divide the
//     CPU clk frequency to give the ADC clk a frequency of 50-200 kHz
//     for 10-bit precision (a higher freq can be used for lower
//     precision and higher sample rates). The Sumovore uses a CPU clk
//     of 1 MHz, so a prescaler of 16 (ADPS2) is sufficient, giving
//     an ADC clk frequency of 62.5 kHz
//   - enable ADC with ADEN bit
// - first ADC conversion (after ADEN set) takes 25 ADC clk
//   cycles, after which ADC conversions take 13 clk cycles.
//   so do an initial (dummy) conversion before actual use.
void setupADC()
{
  // REFS0 (Vref) is AVCC
  // ADLAR=1 as only want magnitude of reflectivity
  ADMUX = (1 << REFS0) | (1 << ADLAR);

  // clk prescaler of 16 (ADPS2) for ADC clk (of 62.5 kHz)
  // and enable ADC with ADEN flag
  ADCSRA = (1 << ADPS2) | (1 << ADEN);

  // do a dummy conversion to complete initialization
  readADC(0);
}



// this sets up 8-bit Timer/Counter 2 to tick over once every 10ms,
// generating an interrupt that increments a global 10ms counter. 
// Note: with a 1MHz CPU clk, and 256 prescalar, 10ms = 39.0625 ticks
// (1sec=3906.25), so if round to 39 ticks, then this gives an error of
// -0.16%, or 1 sec per 10 min 25 sec
// To do this use Clear Timer on Compare Match (CTC) Mode (WGM21:20 = 2),
// in which the counter is cleared to zero when the counter value (TCNT2)
// matches the OCR2 register. OCR2 defines the top value for the counter,
// and so also its resolution.
void setup10msTimer()
{
  // disable interrupts (in case it hasn't already been done)
  cli();

  // set Clear Timer on Compare Match (CTC) mode (TOP = OCR2)
  TCCR2 = (1 << WGM21);

  // set timer clk = CLK/256 -> 1Mhz / 256
  TCCR2 |= (1 << CS22) | (1 << CS21);

  // set timer TOP to 39. ie interrupt every 39 timer cycles ~ 10ms
  OCR2 = 39;

  // enable timer interrupt only on Output Compare Flag 2 (TCNT2 = OCR2)
  TIMSK = (1 << OCIE2);

  // and initialise tickcnt to zero
  tickcnt=0;

  // re-enable interrupts
  sei();
}




/*********************** Line Sensor Calibration Code **********************/



// light green LED indicating robot should be on white background
// indicate with center red LED and read center line sensor
// display level of background surface signal magnitude (0-8) with red LEDs
// flash green to indicate move robot center sensor onto line
// indicate with center red LED and read center line sensor
// display level of black line signal magnitude (0-8) with red LEDs
// calculate threshold as midpoint between the two levels (rounding up)
// display level of line threshold and flash green LED to indicate it
// return black line threshold
//
// note: sensors return a value from 0 - 255
// corresponding to lighter (reflective) to darker (absorbing) surface
uint8_t calibrateLineSensitivity()
{
  // using 8 bits as ADC is only returning high 8-bits of conversion
  uint8_t linesensor_bg;
  uint8_t lvl_bg;
  uint8_t linesensor_line;
  uint8_t lvl_line;
  uint8_t thresh;

  // not using the motors, so turn them off via left and right motor enable
  // but if PWM was already turned on, then this won't work so also set the
  // motors' duty cycles to 0 (STOPPED)
  MOTORPORT &= ~(_BV(LMOTOR_EN) | _BV(RMOTOR_EN));
  RMOTOR_DUTY=LMOTOR_DUTY=STOPPED;

  GREENPORT |= _BV(GREENLED);		      // indicate test (light) surface
  flashred(REDLED_C,2);			      // indicate reading center sensor
  linesensor_bg=readADC(LINESENSOR_C);	      // read center sensor
  lvl_bg=getlevel(linesensor_bg);	      // get magnitude of reading
  outlevel(lvl_bg);			      // and display it
  waitsecs(5);				      // wait for operator to see it
  outlevel(0);				      // clear level display
  GREENPORT &= ~(_BV(GREENLED));	      // indicator off
  //displayvalue(linesensor_bg,5);	      // display actual reading (opt)
  //waitsecs(5);			      // wait for operator to see it

  flashgreen(5);			      // 5 secs to put bot on line

  flashred(REDLED_C,2);			      // indicate reading center sensor
  linesensor_line=readADC(LINESENSOR_C);      // read center sensor
  lvl_line=getlevel(linesensor_line);	      // get magnitude of reading
  outlevel(lvl_line);			      // and display it
  waitsecs(5);				      // wait for operator to see it
  outlevel(0);				      // clear level display

  // if levels OK go for midlevel (rounding up)
  // otherwise assume operator asleep, and go with default
  if (lvl_line>lvl_bg) {	
    thresh=1 << (lvl_bg+((lvl_line-lvl_bg+1)/2));
  } else {			
    thresh=BLACKDEFAULT;
  }

  outlevel(getlevel(thresh));		      // display threshold
  flashgreen(5);			      // indicators showing thresh lvl
  outlevel(0);				      // clear level display

  return thresh;
}



// convert a (sensor reading) value to a (signal strength) level
uint8_t getlevel(uint8_t value)
{
  uint8_t level=0;
  while (value) {
    level++;
    value >>= 1;
  }

  return level;
}



// use Sumovore as a reflectivity level meter for measuring the response
// of the line sensors to different backgrounds, black lines, etc
//
// flash red led indicating which sensor will be read
// continuously loop:
//   read sensor and display magnitude of level
void levelmeter(uint8_t channel)
{
  // using 8 bits as ADC is only returning high 8-bits of conversion
  uint8_t    linesensor;

  flashred(REDLED_L+channel,5);	// channel 0 -> left, .. 4 -> right

  for (;;) {
    linesensor=readADC(channel);	// read sensor
    outlevel(getlevel(linesensor));	// display magnitude of reading
  }
}



// measure the response of the line sensors, displaying the full 8-bit
// values for each (0-225), with lower values corresponding to lighter
// (reflective) surfaces, while higher values correspond to darker
// (absorbing) surfaces
//
// continuously loop:
//   for each line sensor (L to R)
//     flash red LED to indicate which sensor is being read
//     read line sensor
//     display 8-bit signal value with red LEDs (unused red LED flashes):
//   		hi 4 bits (green LED on), pause,
//   		lo 4 bits. 
void lineResponseMeter()
{
  // using 8 bits as ADC is only returning high 8-bits of conversion
  uint8_t linesensor;
  uint8_t i;

  for (;;) {
    for (i=0; i<5; ++i) {			// for each sensor
      flashred(REDLED_L+i,2);			// indicate sensor with red LED
      linesensor = readADC(LINESENSOR_L+i);	// read sensor
      displayvalue(linesensor,5);		// display reading
    }
  }
}



/************************** IR Detector Test Code ***************************/



// test IR detectors - indicate if both are detecting something, with the
// center red LED, or if left or right are detecting with left & right LEDs
// note that IRs return 0 when detect an obstacle, 1 otherwise
void testIRdetectors()
{
  while(1) {
    if (!(IRPIN & (1 << IR_L)) && !(IRPIN & (1 << IR_R))) { // both detect
      REDPORT |= (1 << REDLED_C);
      REDPORT &= ~((1 << REDLED_L) | (1 << REDLED_R));
    } else if (!(IRPIN & (1 << IR_L))) {		    // left detects
      REDPORT |= (1 << REDLED_L);
      REDPORT &= ~((1 << REDLED_C) | (1 << REDLED_R));
    } else if (!(IRPIN & (1 << IR_R))) {		    // right detects
      REDPORT |= (1 << REDLED_R);
      REDPORT &= ~((1 << REDLED_C) | (1 << REDLED_L));
    } else {
      REDPORT &= ~(1 << REDLED_L) | ( (1 << REDLED_C) | (1 << REDLED_R));
    }
  }
}



/**************************** Data Display Code *****************************/



// uses the 5 red LEDs to output a sensor's level from 0-8
// off=0, left -> right = level 1-5, right -> left = level 6-8
// utilises PDx = x (defined in avr/iom8.h), with L-R LEDS being x=2 .. 6 ) 
void outlevel(uint8_t level)
{
  redsoff();			// start with all off = 0

  if (level<6) {		// light from left to right for levels 1-5
    while (level--) {
      PORTD |= (1 << (level+PD2));
    }
  } else {			// >5 light from right to left for 6-8
    level-=5;
    while (level--) {
      PORTD |= (1 << (PD6-level));
    }
  }
}



// display value's bit pattern 4 bits at a time with the red LEDs
// hi 4, pause, lo 4 (with green LED lit to indicate hi 4)
void displayvalue(uint8_t val, uint8_t pause)
{
  uint8_t i;

  // display high 4 bits
  redsoff();
  GREENPORT |= (1 << GREENLED);	// indicate displaying high 4 bits
  for (i=0; i<4; i++) {
    if (val & (1 << 7)) {	// check if highest bit (bit 7) set
      // noting that PD2 is leftmost and PDx is defined as x in avr/iom8.h
      PORTD |= (1 << (PD2+i));
    }
    val <<= 1;			// shift out highest bit
  }
  flashred(REDLED_R,pause);	// indicates pause, and not to read this led
  GREENPORT &= ~(1 << GREENLED);	// turn off hi 4-bit indicator LED

  // display low 4 bits
  redsoff();
  for (i=0; i<4; i++) {
    if (val & (1 << 7)) {	// check if highest bit (bit 7) set
      // noting that PD2 is leftmost and PDx is defined as x in avr/iom8.h
      PORTD |= (1 << (PD2+i));
    }
    val <<= 1;			// shift out highest bit
  }
  flashred(REDLED_R,pause);	// indicates pause, and not to read this led
  redsoff();			// clear LEDs before leaving function
}



/***************************** Inline ASM code ******************************/



// Spin-wait in a loop for the specified number of microseconds
// uses inline Asm
//
// the "volatile" keyword avoids compiler optimising away the code
// asm statement is given in the general format of
// asm(code : output operand list : input operand list [: clobber list]);
//
//   code: "1: sbiw %0,1" "\n\t" "brne 1b"
//   		%0 is first operand (cnt)
//   		\n\t is linefeed tab for readability in generated code
//   output operand list: "=w" (cnt)
//   		operand: cnt
//   		modifiers:
//   			'=' write-only operand (used for all output operands)
//   			'w' use special upper register pairs (r24,r26,r28,r30)
//
//   input operand list: "0" (cnt)
//   		operand: cnt
//   		modifiers: '0' use the same input register as first operand
//
// generated asm code is:
//
// 	1: sbiw r30, 1
// 	brne 1b
//
//   "1:" is a Unix-assembler style local numeric label
//   "sbiw" reg, const: Subtract Immediate (1) from Word (cnt)
//   "brne" label: branch if not equal to label "1" back ('b')
//
void waitUS(uint16_t usec)
{
  register uint16_t cnt;

  // the ASM code takes 4 CLK cycles per loop
  // so for a 1MHz CLK divide by 4, and scale this up for higher CLK rates
  cnt = (usec * (MCU_FREQ / 1000000UL))  / 4;

  if (!cnt) { return; } // otherwise sbiw will decrement cnt to 0xFFFF

  __asm__ volatile (
      "1: sbiw %0,1" "\n\t"
      "brne 1b"
      : "=w" (cnt)
      : "0"  (cnt)
  );
}




